...
import spring.boot.rest.sample.enums.CacheType;
import spring.boot.rest.sample.param.RequestCount;
...
//			if (!limit(new RequestLimit(RemoteAddressUtils.getRealIp(req), req.getRequestURI(), range, count), type)) {
	...
	private boolean limitWithMap(RequestLimit requestLimit) {
		...
		if (!map.containsKey(key)) {
			...
			if (frequency > requestLimit.getRange()) {
				...
				if (requestCount.getCount() >= requestLimit.getCount() && frequency <= requestLimit.getRange()) {
					return false;
				} else {
